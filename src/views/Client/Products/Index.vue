<template>
    <AdminLayout>
        ...
        <div v-if="ready_delivery==1" class="pt-6 md:px-2 px-4" style="min-height:400px">  
            ...
            <hr class="mb-6" />
            <div class="mb-3 text-sm">配送商品 （計{{checked_count}}点）</div>
            <div class="mb-8">
                <template v-for="(item, key) in products">
                    <div  v-if="checks['id'+item.id]" class="mb-0  border-neutral-200 border-b">
            ...
            <div class="mb-8 border border-neutral-200 rounded-md px-4 py-2 flex items-center justify-between">
                ...
                <div>
                    <a href="/address" type="button" @click="back_delivery()" :class="{ 'opacity-25': processing || (!hasCheck) }" :disabled="processing || (!hasCheck)" class="inline-block items-center px-5 py-1.5 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest bg-[#896858] hover:bg-[#60493d] active:bg-neutral-700 focus:outline-none focus:border-neutral-700 focus:shadow-outline-neutral transition ease-in-out duration-150 m-1">
                        編集
                    </a>
                </div>
            </div>
        ...
        <div v-if="ready_delivery==2" class="pt-6 md:px-2 px-4" style="min-height:400px">  
            ...
            <hr class="mb-8" />
            <div class="mb-8 ">
                計{{checked_count}}点の商品の配送手続きが完了しました。<br/><br/>
...
export default {
    ...
    computed: {
        ...mapGetters(['user', 'profile','products', 'products_count']),
    },
    mounted() {
        // let check = {};
    ...
    methods: {
        ...mapActions(['getProducts', 'getProfile', 'exchangeToPoint', 'checkLoginStatus', 'deliverPoint']),
        ...
        changeCheck(id) {
            ...
            if(this.checkboxes.length) {
                this.checked_count = this.checkboxes.length; 
                this.hasCheck = true;
            } else {
            ...
            // console.log('CHECK_BOXES : ' + this.checkboxes, 'COUNTS : ' + this.checked_count);
            value = false;
            index = -1;
        },
        ...
        calcuateCount() {
            ...
            for(let i = 0; i < this.products_count; i++) {
                if (this.checks['id' + this.products[i]['id']]) {
                    console.log(this.checks['id' + this.products[i]['id']])
                    count++; 
            ...
            this.points = point;
            if(this.count) this.hasCheck == false;
        },
        ...
        async onDeliver() {
            if(this.profile.address) {
                if (confirm(this.checked_count + '点の商品を配送しますか？')) {
                    const result = this.deliverPoint({ checks: this.checkboxes, points: this.points});
                    if(result){
                        await this.getProducts(this.tab_id);
                        this.ready_delivery = 0;
                    }
                }
            } else {
