    ...
    <div class="flex min-h-screen text-neutral-700 text-base underline-offset-2 font-medium">
        ...
        <div class="min-h-screen w-full flex flex-col">
            <div class="flex-shrink-0 w-full">
                <div v-if="show_notification" class="bg-red-600 text-sm py-1 text-white">
                    <div class="md:w-[760px] w-full mx-auto md:px-0 px-1">3月1日(水)19〜20時より、サイト内メンテナンスを行います。ご迷惑お掛け致しますが、何卒よろしくお願い申し上げます。</div>
                </div>
                <div class="flex justify-center xl:px-8 px-0 ">
                    <div class="md:w-[760px] w-full flex md:px-0 px-2 mx-auto justify-center md:gap-2 gap-1">
                        <div class="flex items-center flex-1 px-1 md:py-3 py-2">
                            <Logo/>
                        </div>
            ...
            <main class="w-full flex-1 relative">
                <div class="absolute w-full h-full overflow-y-auto" >
                    <div class="relative min-h-full flex flex-col" >
                        <div :class="{'md:w-[760px]': !show_result_bg, 'flex-1 w-full mx-auto bg-no-repeat bg-cover':true}">
                            <div :style="[show_result_bg ? {backgroundImage: result_bg_image} : {}, 'min-height: calc( 100vh - 336px )']" class="bg-no-repeat bg-cover bg-center">
                                <slot />
                            </div>
                        </div>
                        <Footer/>
...
export default {
    components: {Category, Branch, Sidebar, Footer, TransitionRoot, TransitionChild, Dialog, DialogOverlay, XMarkIcon, ChevronLeftIcon, Bars3Icon, Link, Menu, MenuButton, MenuItems, MenuItem,Logo},
    data() {
        return {
            sidebarOpened: false,
